...
export default async function BookVenuePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const supabase = createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) redirect(`/auth/login?redirect=/venues/${id}/book`)
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <Link href={`/venues/${id}`} className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-700 mb-6">
